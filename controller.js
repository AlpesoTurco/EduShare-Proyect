...
controller.get('/message', (req,res)=>{
    const idUserActual = req.session.UserID;
    if (req.session.loggedin) {
        connections.query(`SELECT DISTINCT Nombre, Apellidos, Perfil, Usuario, EstudianteID
        FROM chats
        INNER JOIN Estudiante ON EstudianteID = EstudianteFK1
        INNER JOIN Personas  ON PersonaID = PersonaFK
        WHERE EstudianteFK1 = ? OR EstudianteFK2 = ?
        GROUP BY Nombre, Apellidos, Perfil, Usuario, EstudianteID;        
        `,[idUserActual,idUserActual], (error, results) => {
            if (error) {
                throw error;
            } else {
                res.render('message', {
...
controller.post('/enviarMensaje', async (req, res) => {
    ...
    connections.query('INSERT INTO chats SET ?', [{ComentarioID: IdMensaje, Contenido: Mensaje, FechaHora: FechaHora, EstudianteFK1: Yo, EstudianteFK2: Remitente, ConversacionID: Menso}], async (error, results) => {
        if (error) {
            console.log(error);
            res.status(500).send('Error interno del servidor');
        } else {
